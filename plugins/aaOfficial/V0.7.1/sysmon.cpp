// cl /LD /O2 /GS- /GR- /EHa- /I.. sysmon.cpp /link /ENTRY:DllMain /NODEFAULTLIB kernel32.lib
...
static int api_get_ram(lua_State* L) {
    ...
    lua_pushnumber(L, freeRamMB);
    return 1;
}
extern "C" {
    __declspec(dllexport) int __cdecl lume_plugin_init(lua_State* L, LumeHostAPI* api) {
        g_api = api;
        GetSystemTimes(&prevSysIdle, &prevSysKernel, &prevSysUser);
        lua_register(L, "sysmon_get_cpu", api_get_cpu);
        lua_register(L, "sysmon_get_ram", api_get_ram);
        return 0;
    }
